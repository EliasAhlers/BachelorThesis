...
config = {
    ...
    "benchmark_config": {
        ...
        "use_api_for_transfer": True,  # Set to False to use local model
    },
    "api_config": {
...
judge_system_prompt = "Vergleiche den Inhalt der beiden deutschen Texte und beurteile, ob sie im Wesentlichen die gleiche Bedeutung haben. Achte nur auf den allgemeinen Inhalt, nicht auf spezifische Details, die bei einer Transformation verloren gehen können. Antworte nur mit 'Ja' oder 'Nein'."
...
# Check if two texts have similar content using judge model from api
def judge_content_similarity(original_text: str, transferred_text: str) -> bool:
    openai = OpenAI(
        api_key=config["api_config"]["api_key"],
        base_url=config["api_config"]["base_url"],
    )
    ...
    # print(f"Transferred: {transferred_text}")
    # print(f"Content similarity response: {response}")
    return response == "ja"
...
def run_benchmark(transfer_model, transfer_tokenizer, classification_model, classification_tokenizer) -> Dict:
    ...
    for i, sample in enumerate(test_data, 1):
        ...
        is_content_preserved = judge_content_similarity(original_text, transferred_text)
        print(f"Sample {i}: Original Level: {original_level}, Target Level: {target_level}, Predicted Level: {predicted_level}, Content Preserved: {is_content_preserved}")
        
        if is_transfer_correct:
            correct_transfer += 1
        if is_group_correct:
            group_correct_transfer += 1
        if is_content_preserved:
            content_preserved += 1
        total += 1
        ...
        results.append({
            ...
            "isContentPreserved": is_content_preserved,
            "transferTime": transfer_time
        })

        if i % 10 == 0:
            print(f"Processed {i} samples. Current accuracy: {correct_transfer/total:.2%}, Group accuracy: {group_correct_transfer/total:.2%}, Content preserved: {content_preserved/total:.2%}")

    transfer_accuracy = correct_transfer / total if total > 0 else 0
    group_accuracy = group_correct_transfer / total if total > 0 else 0
    content_preservation_rate = content_preserved / total if total > 0 else 0
    ...
    return {
        ...
        "evaluation": {
            ...
            "contentPreserved": content_preserved,
            "transferAccuracy": transfer_accuracy,
            "groupAccuracy": group_accuracy,
            "contentPreservationRate": content_preservation_rate,
...
def main():
    ...
    print(f"Content Preservation Rate: {benchmark_results['evaluation']['contentPreservationRate']:.2%}")
